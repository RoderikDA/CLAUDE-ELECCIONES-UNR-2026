...
export default function PanelResultados({ config, results, mesasCargadas }) {
  ...
  function getListasConVotos(fid) {
    ...
    return f.listas
      .map(l => ({
        ...l,
        votos: results?.[fid]?.[tipo]?.[l.id]?.votos || 0,
      }))
      .sort((a, b) => b.votos - a.votos);
  }

  function getBlancoNulo(fid) {
    const t = results?.[fid]?.[tipo];
    if (!t) return { blancos: 0, nulos: 0 };
    const blancos = Object.values(t).reduce((a, v) => a + (v.blancos || 0), 0);
    const nulos   = Object.values(t).reduce((a, v) => a + (v.nulos   || 0), 0);
    // blancos/nulos are stored per-lista row but are the same for all, so divide by lista count
    const n = Object.keys(t).length || 1;
    return { blancos: Math.round(blancos / n), nulos: Math.round(nulos / n) };
  }
  ...
  const totalMesasPosibles = config.facultades.reduce((a, f) => a + (f.mesas?.[tipo] ?? 0) * 3, 0);
  const totalMesasCargadas = config.facultades.reduce((a, f) => a + (mesasCargadas?.[f.id]?.[tipo]?.length ?? 0), 0);
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 12, background: "#fff", borderRadius: 12, padding: "12px 16px", boxShadow: "0 1px 6px #0001", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 120, height: 6, background: "#f0f0f0", borderRadius: 99, overflow: "hidden" }}>
            <div style={{
              height: "100%",
              width: totalMesasPosibles > 0 ? `${(totalMesasCargadas / totalMesasPosibles * 100).toFixed(0)}%` : "0%",
              background: "#16a085", borderRadius: 99, transition: "width .5s",
            }} />
          </div>
      ...
      <div style={{ marginTop: 10, marginBottom: 4, display: "flex", justifyContent: "flex-end" }}>
      ...
      {vista === "facultad" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 10 }}>
          {config.facultades.map(f => {
            ...
            const { blancos, nulos } = getBlancoNulo(f.id);
            const cargadas = mesasCargadas?.[f.id]?.[tipo]?.length ?? 0;
            const posibles = (f.mesas?.[tipo] ?? 0) * 3;
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12, alignItems: "flex-start" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#aaa" }}>
                      {cargadas}/{posibles} mesas · {total.toLocaleString()} votos
                    </div>
                  </div>
                  <span style={{
                  ...
                  }}>{total > 0 ? `${Math.round(cargadas / posibles * 100)}%` : "Sin datos"}</span>
                </div>
                  ...
                  <>
                    ...
                    {(blancos > 0 || nulos > 0) && (
                      <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                        {blancos > 0 && (
                          <span style={{ fontSize: 11, background: "#7f8c8d15", color: "#7f8c8d", padding: "3px 10px", borderRadius: 6, fontWeight: 600 }}>
                            Blancos: {blancos}
                          </span>
                        )}
                        {nulos > 0 && (
                          <span style={{ fontSize: 11, background: "#e74c3c15", color: "#e74c3c", padding: "3px 10px", borderRadius: 6, fontWeight: 600 }}>
                            Nulos: {nulos}
                          </span>
                        )}
                      </div>
